...
void createInitialRuns(char *input_file, int run_size, int num_ways)
{
    ...
    while (more_input)
    {
        ...
        {
            if (fscanf(in, "%d,%d,%d,%d", &value1, &value2, &value3, &value4) != 4)
            {
                more_input = false;
                break;
            }
        ...
        sort(arr.begin(), arr.end(), compareByMember1);
...
void mergeFiles(char *output_file, int n, int k)
{
    ...
    {
        char fileName[50];
        snprintf(fileName, sizeof(fileName), "run%d.txt", i);
    ...
    for (i = 0; i < k; i++)
    {
        if (fscanf(in[i], "%d,%d,%d,%d", &val1, &val2, &val3, &val4) != 4)
            break;
        RecordStructure r = {val1, val2, val3, val4};
    ...
    while (count != i)
    {
        ...
        fprintf(out, "%d,%d,%d,%d\n", root.element.member1, root.element.member2, root.element.member3, root.element.member4);
        cout << "Removed value of root from run " << root.i << " is " << root.element.member1 << endl;
        if (fscanf(in[root.i], "%d,%d,%d", &val1, &val2, &val3, &val4) != 4)
        {
